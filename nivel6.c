...
int is_output_redirection(char **args){
    ...
    if( has_output  == 0){                                          //En caso de que no contenga >, devolvemos 0
        return 0;
    }else if(args[has_output] != NULL ){                            //En caso contrario, miramos que el siguiente elemento sea el file.
        int fd = open(args[has_output], O_WRONLY | O_CREAT | O_APPEND);
        if(fd > 0){                                                 //Intentamos abrir el file
            if(dup2(fd,1) == -1){                                   //Intentamos duplicar el descriptor de fichero para redireccionar 
                                                                    //la salida del stdout.
